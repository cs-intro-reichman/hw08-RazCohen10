...
class PlayList {
    ...
    private int minIndex(int start) {
        if (start <= size && start >= 0)
        {
            ...
            {
                if (tracks[currentMinIndex].getDuration() > tracks[i].getDuration())
                {
                    currentMinIndex = i;
                }
            }
    ...
    public void sortedInPlace() {
        ...
        Track trackHolder;
        int trackHolderIndex;
        for (int i = 0; i < size; i++)
        {
            trackHolderIndex = minIndex(i);
            trackHolder = tracks[trackHolderIndex];
            tracks[trackHolderIndex] = tracks[i];
            tracks[i] = trackHolder;
        }
    }
}
